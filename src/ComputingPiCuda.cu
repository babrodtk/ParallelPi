...
#ifdef WIN32
	//It appears that queryperformancecounter does *not* give wall clock anymore?
	LARGE_INTEGER f;
	LARGE_INTEGER t;
...
float computePi(int n_points) {
	...
	const unsigned int kernel_no = 4;
	dim3 grid;
	dim3 block;
